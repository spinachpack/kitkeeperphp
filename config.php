...
function getProfilePicture($filename) {
    $defaultFile = 'default-avatar.jpg'; // or .jpg if that's your file
    $uploadPath = __DIR__ . '/' . PROFILE_UPLOAD_DIR;
    $uploadUrl = BASE_URL . PROFILE_UPLOAD_DIR;

    // if no filename in session or file doesn’t exist, return default
    if (empty($filename) || !file_exists($uploadPath . $filename)) {
        return $uploadUrl . $defaultFile;
    }

    // otherwise return user’s actual uploaded picture
